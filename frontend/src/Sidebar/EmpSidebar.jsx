...
const menuItems = [
  ...
  { title: "Project Management", to: "/employee/project", icon: <Briefcase size={20} /> },
  { title: "Task Management", to: "/employee/task", icon: <CheckSquare size={20} /> },
  { title: "Leave Management", to: "/employee/leave", icon: <Calendar size={20} /> },
  ...
  const SidebarContent = (
    <div className="flex flex-col h-full bg-white border-r border-slate-100 shadow-xl shadow-slate-100/50">
      ...
      <div className={`flex items-center ${collapsed ? "justify-center" : "justify-between"} p-6 mb-4`}>
        ...
        {/* Desktop Toggle Button is usually positioned better as a floating element or part of header */}
      </div>
      ...
      {/* Help/Support Section (Optional for premium feel) */}
      {!collapsed && (
        <div className="mx-4 mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Support Hub</p>
          <p className="text-xs text-slate-600 mb-3 leading-relaxed font-medium">Need assistance with your workspace?</p>
          <button className="w-full py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:bg-slate-100 transition-all shadow-sm">
            Contact HR
          </button>
        </div>
      )}
